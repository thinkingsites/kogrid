define(["knockout","_","jquery"], function (ko,_,$) {
	var rows = _.sortBy([
    ...
    var viewmodel = {
    	live : function(){
    		viewmodel.gridOptions.rows(rows);
    	},
    	template1 : function(){
    		viewmodel.gridOptions.noRows(ko.bindingHandlers.kogrid.options.noRows);
    		viewmodel.gridOptions.rows([]);
    	},
    	template2 : function(){
    		viewmodel.gridOptions.noRows("<div> this is an html template for no rows</div>");
    		viewmodel.gridOptions.rows([]);
    	},
    	template3 : function(){
    		viewmodel.gridOptions.noRows("<div data-bind='style : noRowsStyleForInline '> I'm an inline knockout template!</div>");
    		viewmodel.gridOptions.rows([]);
    	},
    	template4 : function(){
    		viewmodel.gridOptions.noRows("no-rows-callback-template");
    		viewmodel.gridOptions.rows([]);
    	},
	    gridOptions : {
			...
			// noRows accepts an observable and can be changed dynamically
			noRows : ko.observable(ko.bindingHandlers.kogrid.options.noRows),
			
			// custom objects can be added to the grid model and be accessed by the no rows template.  this property is NOT a standard grid option
			noRowsStyleForInline : {
				color : 'red',
				'font-size' : '20px',
				'font-weight' : 'bold'
			},
